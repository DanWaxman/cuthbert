...
def stop_gradient_decorator(func: Resampling) -> Resampling:
    ...
    This wrapper will call the provided resampling function and then stop the gradient
    of the logits.
    ...
    def _wrapped(key: Array, logits: ArrayLike, positions: ArrayTreeLike, n: int):
        idx_base, logits_base, positions_base = func(key, jax.lax.stop_gradient(logits), positions, n)
        logits = logits_base + logits[idx_base] - jax.lax.stop_gradient(logits[idx_base])
